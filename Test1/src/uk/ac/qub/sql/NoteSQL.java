...
public class NoteSQL {
	 ...
	 * The id is the first in the list followed by date, year and the text of
	 * the note
	 * 
	 * @param n
	...
	public static void amendNote(Note n) throws ClassNotFoundException, SQLException {
		...
		String statement = "UPDATE Notes SET date = '" + n.getDate() + "',year_group = " + n.getYear() + ", note_text='"
				+ n.getDetails().replaceAll("'", "''") + "' WHERE id =" + n.getId() + ";";
	...
	/**
	 * This method will delete a note from the SQL database it takes in a String
	 * which will be used to find the id of the Note
	 * 
	 * @param id
	 * @throws ClassNotFoundException
	 * @throws SQLException
	...
	public static void UploadNote(Note n1) throws Exception {
		...
		String Statement = "INSERT INTO Notes (date, year_group, note_text) " + "VALUES ('" + n1.getDate() + "', '"
				+ n1.getYear() + "', '" + n1.getDetails().replaceAll("'", "''") + "')";
	...
	/**
	 * This method will return a list of notes which are active after a certain
	 * date
	 * 
	 * @return
	 * @throws SQLException
