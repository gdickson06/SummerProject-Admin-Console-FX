...
public class PlacementSQL {
		...
		for (Placement p : placements) {
			...
			try {
				newStatement = "INSERT INTO Placements (cohort,module,start_date,end_date,module_number,hospital,preference,comments,year)"
						+ " VALUES ('" + cohort + "', '" + module + "', " + StartDate + ", " + EndDate + ", '"
						+ module_number + "', '" + Location + "', '" + preference + "', '" + comments + "', + " + Year
						+ ")";
	...
	/**
	 * This method will amend a single placement with an amended placement
	...
	public static void amendPlacement(Placement p) throws Exception {
		...
		PreparedStatement statement = null;
		String Statement = "UPDATE Placements SET cohort ='" + p.getCohort().replaceAll("'", "''") + "', module='"
				+ p.getModule().replaceAll("'", "''") + "', start_date='" + p.getStartDate() + "', end_date = '"
				+ p.getEndDate() + "',module_number = '" + p.getModuleNumber().replaceAll("'", "''") + "', hospital ='"
	...
	public static void UploadSinglePlacement(Placement p) throws Exception {
		...
		String Statement = "INSERT INTO Placements(cohort,module,start_date,end_date,module_number,hospital,preference,year) VALUES('"
				+ p.getCohort().replaceAll("'", "''") + "'," + "'" + p.getModule().replaceAll("'", "''") + "','"
	...
	public static File downloadToCSV(String year) throws IOException, SQLException {
		...
		if (r.next()) {
			do {
				...
				sb.append(" ");
				sb.append(',');
				if (r.getString("cohort") == null) {
					sb.append(" ");
				} else {
					sb.append(r.getString("cohort"));
				}
				sb.append(',');
				if (r.getString("module") == null) {
					sb.append(" ");
				} else {
					sb.append(r.getString("module"));
				}
				...
				if (r.getString("start_date") == null) {
					...
					sb.append(r.getString("start_date"));
				}
				...
				if (r.getString("end_date") == null) {
					...
					sb.append(r.getString("end_date"));
				}

				sb.append(',');
				if (r.getString("module_number") == null) {
					sb.append(" ");
				} else {
					sb.append(r.getString("module_number"));
				}

				sb.append(',');
				if (r.getString("hospital") == null) {
					sb.append(" ");
				} else {
					sb.append(r.getString("hospital"));
				}

				sb.append(',');
				if (r.getString("preference") == null) {
					sb.append(" ");
				} else {
					sb.append(r.getString("preference"));
				}

				sb.append(',');
				if (r.getString("comments") == null) {
					sb.append(" ");
				} else {
					sb.append(r.getString("comments"));
				}
