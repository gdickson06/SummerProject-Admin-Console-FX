...
public class AbsenceSQL {
	...
	public static void AmendAbsence(List<String> s) throws Exception {
		// This will be in the order of the id,Studentnumber,lectureid,date,starttime,endtime,reason,approved,type
		...
		String Statement = "UPDATE absence SET StudentNumber =" + s.get(1) + ", LectureID=" + s.get(2) + ", Date= '"
				+ s.get(3) + "', StartTime = '" +s.get(4) +"', EndTime = '" + s.get(5) +"', Reason = '" + s.get(6) +"', Approved = " + s.get(7) + ", Type = '" + s.get(8) + "' WHERE id = " + s.get(0) + ";";
	...
	public static void saveSingleAbsence(List<String> absenceDetails) throws Exception{
		...
		String Statement = "INSERT INTO absence (StudentNumber, LectureID, Date, StartTime, EndTime, Reason, Approved, type)"
				+ "VALUES (" + absenceDetails.get(0) + ", " + absenceDetails.get(1) + ", '" + absenceDetails.get(2) + "', '" + absenceDetails.get(3)
				+ "', '" + absenceDetails.get(4) + "', '" + absenceDetails.get(5) + "', " + absenceDetails.get(6) + ", '" + absenceDetails.get(7)+"');";
